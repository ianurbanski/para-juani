...
//-----------------------//
 ...
 dibujarInstrucciones(){
  ...
  fill(0); // Color blanco para el texto
  textSize(20);
  textAlign(CENTER, CENTER);
  
  if (textos.length > 0) { // Verifica que el texto esté cargado
    text(textos[0], 200, 50, height / 2, width / 2); // Muestra el texto de instrucciones
  }
 ...
 dibujarVictoria(){
  ...
  if (textos.length > 1) {
    text(textos[1], width / 2, 400); // Muestra el texto de victoria
  }
 ...
 dibujarDerrota(){
  ...
  if (textos.length > 2) {
    text(textos[2], width / 2, 400); // Muestra el texto de derrota
  }
 ...
 dibujarCreditos(){
  ...
  if (textos.length > 3) {
    text(textos[3], width / 2, 400); // Muestra los créditos
  }
 ...
 reiniciarJuego() {
  // Restauramos la posición y variables del personaje
  this.personaje.posX = width / 2;
  this.personaje.posY = 420;
  this.personaje.juegoTerminado = false;

  
  // Volvemos a crear enemigos y la recompensa
  this.crearEnemigos();
  this.crearRecompensa();
